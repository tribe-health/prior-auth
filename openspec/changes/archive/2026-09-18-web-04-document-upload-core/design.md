...
This is ordered change `web-04-document-upload-core` in the reviewed `runtime-architecture › web-case-to-letter` plan. The repository already provides verified sessions, clinical command patterns, authorized projection, PGlite/PEM materialization, and responsive React foundations, but this capability is absent or incomplete.

## Goals / Non-Goals

**Goals:** Implement only `document-upload-core` at its focused contract boundary and retain shell-neutral `AppServices`, verified tenant scope, typed browser HTTP contracts, and explicit replica privacy classification before publication.
...
The production `aso_server_axum::api_router` merges the document router. That router mounts multipart upload, metadata read, and command-result lookup and delegates each operation through `AppServices`. Fresh and populated-upgrade integration probes exercised this exact router, verified session resolution, the restricted PostgreSQL adapter, and the local content-addressed store.

There is intentionally no React upload caller in Web-04. Web-05 owns the processing/status feature and its React caller, including canonical page-map and `page_count` materialization. This change therefore completes the mounted browser service boundary without claiming that a user can upload through the rendered interface yet. Tauri and mobile remain deferred until the browser workflow passes Web-17.
