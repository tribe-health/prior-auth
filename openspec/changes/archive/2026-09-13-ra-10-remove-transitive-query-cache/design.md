...
This change implements runtime order independent invariant from the reviewed [phase plan](../../../.kbd-orchestrator/phases/runtime-architecture/plan.md). Source inspection found reusable primitives but no proof that the composed target path works. Read the assessment and its review supplement before editing.

## Goals / Non-Goals

**Goals:** Reconfirm that @assistant-ui/ai-sdk is unused, then remove that dependency edge so its @ai-sdk/react→SWR closure is absent. Preserve the existing assistant-ui presentation surface and exact framework/PEM pins; do not introduce another cache.
...
- Recheck static/dynamic consumers and the resolved SWR dependency chain before removal; stop this change for replanning if an active bridge consumer is found.
- Remove the unused bridge and regenerate only the necessary lock closure.
- Extend the architecture dependency guard to cover transitive paths, prove its negative case, and verify presentation/pins.
...
No general dependency refresh or assistant feature implementation belongs here.
