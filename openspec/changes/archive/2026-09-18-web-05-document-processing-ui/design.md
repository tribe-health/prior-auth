...
This is ordered change `web-05-document-processing-ui` in the reviewed `runtime-architecture › web-case-to-letter` plan. The repository already provides verified sessions, clinical command patterns, authorized projection, PGlite/PEM materialization, and responsive React foundations, but this capability is absent or incomplete.

## Goals / Non-Goals

**Goals:** Implement only `document-processing-ui` at its focused contract boundary and retain shell-neutral `AppServices`, verified tenant scope, typed browser HTTP contracts, and explicit replica privacy classification before publication.
...
The production browser route `/cases/:caseId/intake` loads `CaseIntake`, which mounts `DocumentIntake`. Upload commands pass through `useDocumentUpload` and the typed multipart HTTP client. Status reads pass through `useDocumentStatuses`, which joins the committed `DocumentStatus` entities and ordered identifiers in the PEM graph. The Axum application mounts upload, metadata, processor, and command-lookup routes over the same shell-neutral `AppServices` operations.

Processing remains a job-principal operation. React does not call the processor route and cannot mint the required job grant. Web-16 must invoke the real mounted processor endpoint from its synthetic job runner before Web-17 can certify the browser workflow.

## Uncomfortable limitation

This change proves the mounted boundary and focused lifecycle, but it does not prove a continuously running production scheduler or an actual-browser upload-to-ready transition. No such readiness claim is made here. Web-16 owns deterministic orchestration and Web-17 owns the unchanged-candidate browser campaign.
