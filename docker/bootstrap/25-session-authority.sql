...
-- ASO owns the durable deployment/session denial journal and ordered authority
-- outbox. These local control relations never join Electric replication.
-- Membership/capability triggers advance the global revision and append its
-- outbox event in the same transaction. Restore procedures rotate the ASO
-- authority incarnation before serving restored data.
...
REVOKE ALL ON FUNCTION aso.emit_session_denial_event(),
  aso.protect_session_denial_retention(), aso.protect_session_denial_truncate()
  FROM PUBLIC;
...
GRANT SELECT ON aso.users, aso.user_roles, aso.role_capabilities,
  aso.capabilities, aso.user_capabilities, aso.authority_deployment,
  aso.authorization_revision, aso.session_denials TO aso_session_reader;
