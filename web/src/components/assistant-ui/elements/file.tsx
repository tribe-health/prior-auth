...
const fileVariants = cva(
  "aui-file-root inline-flex items-center gap-3 rounded-lg transition-colors",
  {
    variants: {
      variant: {
        outline: "border-border hover:bg-ui-muted/50 border",
        ghost: "hover:bg-ui-muted/50",
        muted: "bg-ui-muted/50 hover:bg-ui-muted/70",
      },
      size: {
...
function FileIconDisplay({
  ...
  return (
    <span
      data-slot="file-icon"
      className={cn("text-ui-muted-foreground shrink-0", className)}
      {...props}
    >
...
function FileDownload({
  ...
  return (
    <a
      ...
      className={cn(
        "text-ui-muted-foreground hover:bg-accent hover:text-accent-foreground shrink-0 rounded-md p-1 transition-colors",
        className,
      )}
      {...props}
    >
