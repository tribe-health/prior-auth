  ...
  return (
    <div data-slot="aui_thread-list-search" className="relative px-0.5 py-1">
      <SearchIcon
        ...
        className="text-ui-muted-foreground pointer-events-none absolute start-3 top-1/2 size-4 -translate-y-1/2"
      />
      <Input
        ref={ref}
        type="search"
  ...
  if (query && filteredIndices.length === 0) {
    return (
      <div
        ...
        className="text-ui-muted-foreground px-2.5 py-4 text-sm"
      >
        No threads found
      </div>
    );
  ...
  return groups.map((group) => (
    <Fragment key={group.label}>
      <div
        ...
        className="text-ui-muted-foreground px-2.5 pt-3 pb-1 text-xs font-medium"
      >
        {group.label}
      </div>
...
>(({ className, labelClassName, children, ...props }, ref) => {
  return (
    <ThreadListPrimitive.New render={<Button ref={ref} variant="ghost" data-slot="aui_thread-list-new" className={cn(
                "hover:bg-ui-muted data-active:bg-ui-muted h-8 justify-start gap-2 rounded-md px-2.5 text-sm font-normal",
                className,
              )} {...props} />}>{children ?? (
                <>
                  <PlusIcon
...
export const ThreadListItem: FC = () => {
  ...
  return (
    <ThreadListItemPrimitive.Root
      ...
      className="group hover:bg-ui-muted focus-visible:bg-ui-muted data-active:bg-ui-muted has-focus-visible:bg-ui-muted has-data-[state=open]:bg-ui-muted relative flex h-8 items-center rounded-md transition-colors focus-visible:outline-none"
    >
      {isRenaming ? (
        ...
        >
          {isRunning && (
            <Loader2Icon
              ...
              className="text-ui-muted-foreground me-1.5 size-3.5 shrink-0 animate-spin"
            />
          )}
          <span
